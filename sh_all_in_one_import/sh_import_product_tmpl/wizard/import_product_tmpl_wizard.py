        ...
        if self and self.file:
            if self.import_type == 'csv' or self.import_type == "excel":
                ...
                try:
                    ...
                    for row in values:
                        try:
                            ...
                            if row[0].strip() not in (None, ""):
                                vals = {}
                                if row[1].strip() not in ("", None, False) and row[1].strip() == 'FALSE':
                                    vals.update({
                                        'sale_ok': False,
                                    })
                                elif row[1].strip() not in ("", None, False) and row[1].strip() == 'TRUE':
                                    vals.update({
                                        'sale_ok': True,
                                    })
                                if row[2].strip() not in ("", None, False) and row[2].strip() == 'FALSE':
                                    vals.update({
                                        'purchase_ok': False,
                                    })
                                elif row[2].strip() not in ("", None, False) and row[2].strip() == 'TRUE':
                                    vals.update({
                                        'purchase_ok': True,
                                    })
                                if row[3].strip() not in ("", False, None) and row[3].strip() == 'Goods':
                                    vals.update({
                                        'type': 'consu'
                                    })
                                elif row[3].strip() not in ("", False, None) and row[3].strip() == 'Service' or row[3].strip() == 'Service':
                                    vals.update({
                                        'type': 'service'
                                    })
                                elif row[3].strip() not in ("", False, None) and row[3].strip() == 'Combo':
                                    vals.update({
                                        'type': 'combo'
                                    })
                                categ_id = False
                                ...
                                if created_product_tmpl and created_product_tmpl.product_variant_id and created_product_tmpl.type == 'consu' and row[17] != '':
                    ...
                    raise UserError(
                        _("Sorry, Your file does not match with our format")) from exc
            ...
            raise UserError(('Please Attach The File !'))
