        ...
        if self and self.file:
            ...
            if self.import_type in ('csv', 'excel'):
                ...
                try:
                            ...
                            if self.attendance_by == 'badge':
                                ...
                                if row[1] != '':
                                    if row[1]:
                                        ...
                                        naive = datetime.datetime.strptime(
                                            check_in_time, DEFAULT_SERVER_DATETIME_FORMAT)
                                        local_dt = local.localize(
                                            naive, is_dst=None)
                                        utc_dt = local_dt.astimezone(pytz.utc)
                                        check_in_time = utc_dt.strftime(
                                ...
                                if row[2] != '':
                                    if row[2]:
                                        ...
                                        local = pytz.timezone(self.env.user.tz)
                                        naive = datetime.datetime.strptime(
                                            check_out_time, DEFAULT_SERVER_DATETIME_FORMAT)
                                        local_dt = local.localize(
                                            naive, is_dst=None)
                                        utc_dt = local_dt.astimezone(pytz.utc)
                                        check_out_time = utc_dt.strftime(
                                            DEFAULT_SERVER_DATETIME_FORMAT)
                                    else:
                                        skipped_line_no[str(
                    ...
                    raise UserError(
                        _("Sorry, Your csv file does not match with our format")) from e
                if counter > 1:
