        ...
        if self and self.file:
            ...
            if self.import_type == 'csv' or self.import_type == 'excel':
                ...
                try:
                    ...
                    for row in values:
                        try:
                            ...
                            if row[0].strip() not in (None, ""):
                                vals = {}
                                if row[1].strip() not in ("", False, None) and row[1].strip() == 'FALSE':
                                    vals.update({
                                        'sale_ok': False,
                                    })
                                elif row[1].strip() not in ("", False, None) and row[1].strip() == 'TRUE':
                                    vals.update({
                                        'sale_ok': True,
                                    })
                                ...
                                elif row[3].strip() not in ("", False, None) and row[3].strip() == 'Service' or row[3].strip() == 'Storable Product':
                                    vals.update({
                                        'type': 'service',
                                    })
                    ...
                    raise UserError(
                        _("Sorry, Your csv file does not match with our format")) from exc
