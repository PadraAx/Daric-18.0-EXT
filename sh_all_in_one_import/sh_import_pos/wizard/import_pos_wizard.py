                            ...
                            if row[0] not in (None, "") and row[5] not in (None, ""):
                                ...
                                if row[0] != '':
                                    ...
                                    if row[2] not in (None, ""):
                                        ...
                                        pos_vals.update({'partner_id': False})

                                    if row[3] not in (None, ""):
                                        cd = row[3]
                                        user_tz = pytz.timezone(
                                            self.env.user.tz or self.env.context.get('tz') or 'UTC')
                                        dt = user_tz.localize(fields.Datetime.from_string(
                                            cd)).astimezone(pytz.timezone('UTC'))
                                    ...
                                    if search_product:
                                        ...
                                        if row[10].strip() in (None, "") and search_product.taxes_id:
                                            ...
                                            if taxes_list:
                                                ...
                                                vals.update(
                                                    {'tax_ids': [(6, 0, '')]})
                                        price_subtotal = float(float(row[9]))
                                        price_subtotal_incl = float(
                                            float(row[9]))
                                        if vals.get('tax_ids', False):
                                            if vals.get('tax_ids')[0] and vals.get('tax_ids')[0][2] not in (None, ''):
                                                tax_obj = self.env['account.tax'].search(
                                                    [('id', 'in', vals.get('tax_ids')[0][2])])
                                                compute_tax = tax_obj.compute_all(float(
                                                    row[9]), product=search_product.product_tmpl_id, partner=self.env['res.partner'])
                                                price_subtotal = compute_tax['total_excluded']
                                                price_subtotal_incl = compute_tax['total_included']

                                        vals.update({'price_subtotal': float(price_subtotal) * float(
                                            row[7]), 'price_subtotal_incl': price_subtotal_incl * float(row[7])})
                                        vals.update(
