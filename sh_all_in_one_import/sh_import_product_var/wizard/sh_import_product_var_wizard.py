...
class import_product_var_wizard(models.TransientModel):
    ...
    def validate_field_value(self, field_name, field_ttype, field_value,
        ...
        try:
            ...
            _logger.warning(
                "%s: This type of field has no validation method", field_ttype)
            return {}
        return checker(field_name, field_ttype, field_value,
                       field_required, field_name_m2o)

    def validate_field_many2many(self, field_name, field_value,
                                 field_required, field_name_m2o):
        self.ensure_one()
    ...
    def validate_field_many2one(self, field_name, field_value,
                                field_required, field_name_m2o):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
    ...
    def validate_field_text(self, field_name, field_value,
                            field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
    ...
    def validate_field_html(self, field_name, field_value,
                            field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
    ...
    def validate_field_integer(self, field_name, field_value,
                               field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
    ...
    def validate_field_float(self, field_name, field_value,
                             field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
    ...
    def validate_field_char(self, field_name, field_value,
                            field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
    ...
    def validate_field_selection(self, field_name, field_value,
                                 field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
        ...
        )._fields[field_name].get_values(self.env)
        ...
        if selection_key_value_list and field_value not in (None, ""):
            for tuple_item in selection_key_value_list:
                if tuple_item == field_value:
                    return {field_name: tuple_item or False}
                        ...
                        try:
                            ...
                            if row[0] not in (None,
                                ...
                                if row[0] != running_tmpl:
                                    ...
                                    if row[4].strip() == 'Goods':
                                        tmpl_vals.update(
                                            {'type': 'consu'})
                                    elif row[4].strip() == 'Service':
                                        tmpl_vals.update({'type': 'service'})
                                    elif row[4].strip() == 'Combo':
...
#                                             continue
                                    ...
                                    if row[12] not in (None, "", 0, 0.0, '0', '0.0'):
                                        tmpl_vals.update(
...
#                                         created_product_tmpl.attribute_line_ids = False
                                    ...
                                    if has_variant is False and row[
                                            20] not in (None, ""):
                                    ...
                                    if row[14].strip() not in (
                                        ...
                                        if self.method == 'create':
                                            ...
                                            if created_product_tmpl.product_variant_ids:
                                                ...
                                                if row[22] not in (None, "", 0, 0.0, '0', '0.0'):
                                                    var_vals.update({
                                                        'standard_price':
                                                        float(row[22])
                                                    })
                                    ...
                                    if created_product_tmpl:
                                        ...
                                        for k_row_index, v_field_dic in row_field_dic.items(
                                            ...
                                            if dic.get("error", False):
                                                ...
                                                is_any_error_in_dynamic_field = True
                                                break
                                            dynamic_vals.update(dic)
                                        if is_any_error_in_dynamic_field:
                                            counter = counter + 1
                                            continue
                                        created_product_tmpl.write(
                                            dynamic_vals)
                                counter = counter + 1
                            else:
                                skipped_line_no[str(
