...
class ImportUserWizard(models.TransientModel):
    ...
    def validate_field_many2many(self, field_name, field_value, field_required, field_name_m2o):
        ...
        name_relational_model = self.env['res.users'].fields_get()[field_name]['relation']
        ids_list = []
    ...
    def import_user_apply(self):
        "Import User By csv and Excel"
        user_obj = self.env['res.users']
        ir_model_fields_obj = self.env['ir.model.fields']
        if self and self.file and self.company_id.id:
            # For CSV
            if self.import_type in ['csv', 'excel']:
                ...
                try:
                                    ...
                                    if row[3] != '':
                                        ...
                                        if ',' in group:
                                            ...
                                            category_id = self.env['ir.module.category'].sudo().search(
                                                [('name', '=', split_str[0].strip())], limit=1)
                                            if category_id:
                                                print(
                                                    "\n\n\n....category_id...", category_id)
                                                group_id = self.env['res.groups'].sudo().search(
                                                    [('name', '=', split_str[1].strip()), ('category_id', '=', category_id.id)], limit=1)
                                                print(
                                                    "\n\n\\n..group_id...", group_id)
                                                if group_id:
                    ...
                    raise UserError(_('Sorry, Your csv/Excel file does not match with our format')) from exc
            ...
            raise UserError(_("Please Attach The File Or Select Company!"))
