...
class ImportProductVarMBWizard(models.TransientModel):
    ...
    def validate_field_many2many(self, field_name, field_value, field_required, field_name_m2o):
        self.ensure_one()
                ...
                for x in field_value.split(','):
                    ...
                    if x != '':
                        ...
                        if self.is_create_m2m_record and not record:
                            try:
                                record = self.env[name_relational_model].sudo().create({
                                    field_name_m2o: x})
    ...
    def validate_field_many2one(self, field_name, field_value, field_required, field_name_m2o):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
        else:
    ...
    def validate_field_text(self, field_name, field_value, field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
        return {field_name: field_value or False}

    def validate_field_integer(self, field_name, field_value, field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
        return {field_name: field_value or False}

    def validate_field_float(self, field_name, field_value, field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
        return {field_name: field_value or False}

    def validate_field_char(self, field_name, field_value, field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
        return {field_name: field_value or False}

    def validate_field_boolean(self, field_name, field_value,):
        self.ensure_one()
    ...
    def validate_field_selection(self, field_name, field_value, field_required):
        self.ensure_one()
        if field_required and field_value in (None, ""):
            return {"error": " - " + field_name + " is required. "}
        ...
        for rowx, row in enumerate(map(sheet.row, range(sheet.nrows)), 1):
            ...
            for colx, cell in enumerate(row, 1):
                if cell.ctype is xlrd.XL_CELL_NUMBER:
                    ...
                    values.append(
                        dt.strftime(DEFAULT_SERVER_DATETIME_FORMAT) if is_datetime else dt.
                        strftime(DEFAULT_SERVER_DATE_FORMAT))
                elif cell.ctype is xlrd.XL_CELL_BOOLEAN:
                    values.append('True' if cell.value else 'False')
                elif cell.ctype is xlrd.XL_CELL_ERROR:
                    raise ValueError(_("Invalid cell value at row %(row)s, column %(col)s: %(cell_value)s"
                                       ) % {
                        'row':
                        rowx,
                        'col':
                        ...
                        try:
                            ...
                            if row[0] not in (None, "") and row[1] not in (None, ""):
                                ...
                                if row[0] != running_tmpl:
                                    ...
                                    if row[2].strip() not in ("", False, None) and row[2].strip() == 'FALSE':
                                        tmpl_vals.update({'sale_ok': False})
                                    elif row[2].strip() not in ("", False, None) and row[2].strip() == 'TRUE':
                                        tmpl_vals.update({'sale_ok': True})
                                    if row[3].strip() not in ("", False, None) and row[3].strip() == 'FALSE':
                                        tmpl_vals.update(
                                            {'purchase_ok': False})
                                    elif row[3].strip() not in ("", False, None) and row[3].strip() == 'TRUE':
                                        tmpl_vals.update({'purchase_ok': True})
                                    if row[4].strip() not in ("", None, False) and row[4].strip() == 'Goods':
                                        tmpl_vals.update(
                                            {'type': 'consu', 'is_storable': True})
                                        print('\n\n\n\n wblkfnikk', tmpl_vals)
                                    elif row[4].strip() not in ("", None, False) and row[4].strip() == 'Service' or row[4].strip() == 'Service':
                                        tmpl_vals.update({'type': 'service'})
                                    elif row[4].strip() not in ("", None, False) and row[4].strip() == 'Combo':
                                        tmpl_vals.update({'type': 'combo'})
                                    ...
                                    if row[5].strip() in (None, ""):
                                        ...
                                        search_category = self.env['product.category'].search([('complete_name', '=',
                                                                                                row[5].strip())], limit=1)
                                    ...
                                    if row[13] not in (None, "", 0, 0.0, '0', '0.0'):
                                        tmpl_vals.update(
                                    ...
                                    if row[14].strip() in (
                                        ...
                                        if row[18] not in (None, "", 0, 0.0, '0', '0.0'):
                                            tmpl_vals.update(
                                                {'weight': float(row[18])})

                                        if row[19] not in (None, "", 0, 0.0, '0', '0.0'):
                                            tmpl_vals.update(
                                                {'volume': float(row[19])})
                                        ...
                                        if row[21].strip() not in (None, ""):
                                            ...
                                            if "http://" in image_path or "https://" in image_path:
                                                try:
                                                    r = requests.get(
                                                        image_path, timeout=10)

                                                    if r and r.content:
                                                        image_data = BytesIO(
                                                            r.content)
                                                        tmpl_vals.update({
                                                            'image_1920': base64.b64encode(image_data.read())
                                                        })
                                                    else:
                                                        skipped_line_no[str(
                                                            counter
...
#                                         created_product_tmpl.attribute_line_ids = False
                                    ...
                                    if has_variant == False and row[
                                            20] not in (None, ""):
                                        if created_product_tmpl and created_product_tmpl.product_variant_id and created_product_tmpl.type == 'consu':
                                            stock_vals = {
                                                'product_tmpl_id':
                                                created_product_tmpl.id,
                                                'new_quantity':
                                        ...
                                        if self.method == 'create':
                                            ...
                                            if created_product_tmpl.product_variant_ids:
                                                ...
                                                if row[19] not in (None, "", 0, 0.0, '0', '0.0'):
                                                    var_vals.update(
                                                        {'volume': float(row[19])})
                                                if row[23] not in (None, "", 0, 0.0, '0', '0.0'):
                                                    var_vals.update({
                                                        'standard_price':
                                                        float(row[23])
                                                    })
                                                ...
                                                if product_varient.type == 'consu' and row[
                                                        20] != '':
                                                    stock_vals = {
                                                        'product_tmpl_id':
                                                        created_product_tmpl.
