...
class ImportINVWizard(models.TransientModel):
    ...
    def read_xls_book(self):
        """Read CSV or XLS file which is automatically formated using this method"""
                                ...
                                if created_inv:
                                    ...
                                    if search_product:
                                        ...
                                        if self.account_option == 'default':
                                            ...
                                            account_id = self.env['account.account'].sudo().search([('code','=',row[3]),('company_ids','=',self.env.company.id)],limit=1)
                                            if account_id:
                                                account = account_id
                                        if not account:
                                            skipped_line_no[str(
                                            ...
                                            for x in row[9].split(','):
                                                ...
                                                if x != '':
                                                    search_tax = self.env['account.tax'].search(
                                                        [('name', '=', x),('company_id','=',self.env.company.id)], limit=1)
                                                    if search_tax:
                                                        taxes_list.append(
                                                            search_tax.id)
                                                    else:
...
#                   here call necessary method
                    if created_inv_list_for_validate:
